...
export default function AudioPage() {
  ...
  useEffect(() => {
    if (!transcriptData?.original_file_retained) {
      setAudioUrl(null)
      return
    }
    let active = true
    let objectUrl: string | null = null
    apiClient.getAudioFile(transcriptData.id).then((blob) => {
      if (!active) return
      objectUrl = URL.createObjectURL(blob)
      setAudioUrl(objectUrl)
    }).catch(() => active && setAudioUrl(null))
    return () => {
      active = false
      if (objectUrl) URL.revokeObjectURL(objectUrl)
    }
  }, [transcriptData?.id, transcriptData?.original_file_retained])
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 space-y-6">
            <Card className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 rounded-2xl p-4 flex flex-col items-center justify-center border border-gray-100">
                ...
                <audio
                  ...
                  controls
                />
                {!transcriptData.original_file_retained ? (
                  <p className="text-xs text-gray-500 mt-3 text-center">Original audio was removed after transcription according to your settings.</p>
                ) : null}
              </div>
            ...
            <Card className="p-6 space-y-4">
              ...
              {audioUrl ? (
                <a href={audioUrl} download={transcriptData.file_name}
                  className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors text-left">
                  <div className="flex items-center gap-3"><FileAudio className="w-5 h-5 text-gray-500" />
                    <div><p className="font-medium text-gray-900 text-sm">Original Audio</p><p className="text-xs text-gray-500">Private uploaded file</p></div></div>
                  <Download className="w-4 h-4 text-gray-400" />
                </a>
              ) : null}
            </Card>
